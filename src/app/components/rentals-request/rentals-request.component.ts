...
@Component({
  selector: 'app-rentals-request',
  templateUrl: './rentals-request.component.html',
  styleUrls: ['./rentals-request.component.scss']
})
export class RentalsRequestComponent {
  itemsPerPage = 5
  res: any = []
  columna = 'codigo'
  ordenamiento = 'asc'
  ...
  getReserveRequests() {
   ...
   // console.log("id usuario", idUser)
      this.rent.getfilterIdUser(idUser).subscribe(data => {
         this.res= data
  ...
  cambiarPaginacion(key: string, event: any) {
    ...
    console.log("columna es:", this.columna, "ordenamiento es:", this.ordenamiento, "busqueda es ", this.busqueda),
      ...
      this.rent.getfilterIdUser(1).subscribe(data => {  // tener en cuenta por los paraetro la busqueda 
